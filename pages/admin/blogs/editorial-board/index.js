...
const TeamPage = () => {
  ...
  const handleAddMember = async (newMember) => {
    try {
      const response = await axios.post("/api/v1/blogs/editorial", newMember);
      setTeamMembers((prev) => {
  ...
  const handleSubmitEdit = async (updatedMember) => {
    try {
      const response = await axios.put(
        `/api/v1/blogs/editorial/${updatedMember.memberID}`,
        updatedMember
      );
  ...
  const handleDeleteMember = async (memberID) => {
    try {
      await axios.delete(`/api/v1/blogs/editorial/${memberID}`);
      setTeamMembers((prev) => {
        const newMembers = { ...prev };
